...
class PreparedCompactionStore:
        ...
        *,
        ...
        with self._writer_coordinator.transaction(
            ...
            if existing is not None:
                ...
                if batch.state != "failed" or (batch.next_retry_at or 0) > now:
                    return batch, None
    ...
    def status(self, conversation_id: str = "", *, enabled: bool = False, worker_enabled: bool = False) -> dict[str, Any]:
        ...
        with self._lock:
            ...
            last = self._conn.execute(
                """
                SELECT rejected_reason, last_error FROM lcm_prepared_compactions
        ...
        return {
            ...
            "last_rejected_reason": (str(last["rejected_reason"] or "") if last else "") or None,
            "last_error": (str(last["last_error"] or "") if last else "") or None,
        }
...
class BackgroundCompactionScheduler:
    ...
    def _run_job(job: _ScheduledJob) -> None:
        ...
        if engine is None:
            return
        engine.prepare_background_compaction_once(job.messages, _scheduled=True)
...
class BackgroundCompactionMixin:
    ...
    def prepare_background_compaction_once(
        ...
        *,
        ...
        batch, attempt_token = self._prepared_compactions.claim(
            capture,
        ...
        try:
            with heartbeat:
                ...
                self._leaf_publication_capture_callback = recapture
                try:
                ...
                earliest_at, latest_at = self._store.get_time_bounds(intent.source_store_ids)
            ...
            return self._prepared_compactions.mark_ready(
                batch.batch_id,
                attempt_token,
                prepared_intent,
            )
        except Exception as exc:
            return self._prepared_compactions.mark_failed(
                batch.batch_id,
                attempt_token,
                exc,
