...
def test_eleven_engine_clones_share_one_writer_and_preserve_exact_rows(tmp_path: Path):
    ...
    prototype = LCMEngine(config=LCMConfig(database_path=str(db_path)))
    engines = [prototype, *(prototype.clone_for_agent() for _ in range(10))]
    coordinator = prototype._store.writer_coordinator
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=len(engines)) as pool:
            list(pool.map(write_clone, range(len(engines))))

        conn = sqlite3.connect(db_path)
        try:
            assert conn.execute("SELECT COUNT(*) FROM messages").fetchone()[0] == 11
            assert conn.execute("SELECT COUNT(*) FROM summary_nodes").fetchone()[0] == 11
            assert conn.execute("SELECT COUNT(*) FROM lcm_lifecycle_state").fetchone()[0] == 11
            assert conn.execute(
    ...
    assert closed_metrics["checkpoint_count"] == 1
