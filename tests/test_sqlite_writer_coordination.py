...
def _wait_for_waiting_writer(coordinator, timeout: float = 1.0) -> bool:
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        with coordinator._condition:
            if coordinator._next_ticket > coordinator._serving_ticket + 1:
                return True
        time.sleep(0.005)
    return False


@pytest.mark.parametrize("writer_kind", ["rollup", "vector", "command"])
def test_optional_same_database_writers_use_canonical_admission(
    tmp_path: Path,
    writer_kind: str,
):
    db_path = tmp_path / f"optional-{writer_kind}.db"
    store = MessageStore(db_path)
    rollups = RollupStore(db_path) if writer_kind == "rollup" else None
    vectors = VectorStore(db_path) if writer_kind == "vector" else None
    command_conn = None
    if writer_kind == "command":
        command_conn = sqlite3.connect(db_path, timeout=0.05, check_same_thread=False)
    coordinator = store.writer_coordinator
    finished = threading.Event()
    errors: list[BaseException] = []

    def write() -> None:
        try:
            if writer_kind == "rollup":
                assert rollups is not None
                rollups.upsert_building("day", "2026-07-21", "scope")
            elif writer_kind == "vector":
                assert vectors is not None
                vectors.register_profile("test-model", "test-provider", 2)
            else:
                assert command_conn is not None
                _ensure_inflight_table(command_conn)
        except BaseException as exc:
            errors.append(exc)
        finally:
            finished.set()

    thread = threading.Thread(target=write, name=f"optional-{writer_kind}-writer")
    try:
        with store.write_transaction():
            store.connection.execute(
                "INSERT INTO metadata(key, value) VALUES(?, ?)",
                (f"hold-{writer_kind}", "1"),
            )
            thread.start()
            assert _wait_for_waiting_writer(coordinator), (
                f"{writer_kind} writer bypassed the canonical coordinator"
            )
...
def test_eleven_engine_clones_share_one_writer_and_preserve_exact_rows(tmp_path: Path):
    ...
    try:
        ...
        assert all(engine._prepared_compactions.writer_coordinator is coordinator for engine in engines)
        helpers_per_engine = 5
        assert coordinator.metrics_snapshot()["owner_count"] == engine_count * helpers_per_engine
