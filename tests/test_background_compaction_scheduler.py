...
def test_promoting_prepared_compaction_immediately_drains_rollup_invalidation(
    tmp_path,
    monkeypatch,
):
    monkeypatch.setattr(
        "hermes_lcm.engine.summarize_with_escalation",
        lambda **_kwargs: ("prepared rollup-invalidating summary", 1),
    )
    db_path = tmp_path / "prepared-rollup-invalidation.db"
    engine = _engine(db_path, 0)
    engine._config.async_background_compaction_worker_enabled = False
    engine._config.temporal_rollups_enabled = True
    messages = _messages("prepared-rollup-invalidation")
    rollups = RollupStore(db_path)
    day = datetime.now(timezone.utc).date().isoformat()
    scope = "scheduler-session-0"
    try:
        token = rollups.upsert_building("day", day, scope)
        assert rollups.mark_ready(token, "old daily rollup", 1, [], "old") is True
        engine.ingest(messages)
        ...
        assert rollups.has_pending_invalidations() is False
        assert rollups.get_rollup("day", day, scope)["status"] == "stale"
    finally:
        rollups.close()
        engine.shutdown()
